-- Między Nami v0.9.7
-- „Co wybierasz?” na dwóch telefonach w trybie na żywo.
-- Każde wybiera jedną z dwóch odpowiedzi, a wynik odsłania się po ruchu obojga.

-- Rozszerzamy wspólny silnik sesji o dilemma_live (10 pytań).
do $$
declare
  item record;
begin
...
alter table public.multiplayer_sessions
  add constraint multiplayer_sessions_game_key_v097_check
  check (
    game_key in (
      'spicy_match',
      'spicy_desire',
...
alter table public.multiplayer_sessions
  add constraint multiplayer_sessions_question_count_v097_check
  check (
...
begin
  ...
  if p_game_key not in ('who_live', 'know_live', 'dilemma_live') then
  ...
  -- Zachowujemy tylko jedną tymczasową sesję danego trybu na parę.
...
begin
  ...
  if target.game_key not in ('who_live', 'know_live', 'dilemma_live') then
    raise exception 'INVALID_GAME_KEY';
  end if;
