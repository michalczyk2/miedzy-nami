-- Między Nami v0.9.4
-- „Znasz mnie?” na dwóch telefonach w trybie na żywo.
-- Role zmieniają się co kartę: jedna osoba odpowiada o sobie,
-- a druga próbuje przewidzieć jej odpowiedź.

-- Rozszerzamy wspólny silnik sesji o know_live (10 pytań).
do $$
declare
  item record;
begin
...
alter table public.multiplayer_sessions
  add constraint multiplayer_sessions_game_key_v094_check
  check (
    game_key in (
      'spicy_match',
      'spicy_desire',
...
alter table public.multiplayer_sessions
  add constraint multiplayer_sessions_question_count_v094_check
  check (
...
begin
  ...
  if p_game_key not in ('who_live', 'know_live') then
...
begin
  ...
  if target.game_key not in ('who_live', 'know_live') then
    raise exception 'INVALID_GAME_KEY';
  end if;
